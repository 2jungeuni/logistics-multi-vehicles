...
def optimize(data, working_time=None, capacity=None, penalty=None):
    ...
    if capacity:
        cons6 = m.addConstrs(gp.quicksum(-1 * p_vars[i, v] * p[(i, v)] for i in range(n)) <= capacity
                            for v in range(num_v))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Write delivery calls, the number of drivers, working time of delivers, "
                                                 "capacity of vehicles for delivery, and penalty given "
                                                 "for not visiting a location")
    parser.add_argument("-v", "--vehicle", type=int, required=True, help="write the number of drivers")
    ...
    # make problem
    num_veh = args.vehicle  # number of vehicles
    ...
    # print result
    print_result = {"Driver ID": list(np.arange(num_veh)),
                    "Path": routes,
                    "# of serviced packages": pack,
                    "Traveling time": tt}
    print(tabulate(print_result, headers="keys", tablefmt="fancy_grid", missingval="N/A"))

    visualization(RoutingPlanner(0, 1, 0), routes, num_veh)